---
layout: archive
title: "CV"
permalink: /cv/
author_profile: false
redirect_from:
  - /resume
---

{% include base_path %}

Download my CV [here](/files/cv/cv.pdf){:target="_blank"}

Education
======

- **Ph.D. in Mathematics (2018-2022)**
    - **Institution**: Instituto de Matemática Pura e Aplicada (IMPA)
    - **Location**: Rio de Janeiro, Brazil
    - **Thesis**: Boosting and concentration of measure methods in Machine Learning. [(PDF)](https://impa.br/wp-content/uploads/2022/10/dout_tese_Thiago_Rodrigo_Ramos.pdf){:target="_blank"} [(Video)](https://www.youtube.com/watch?v=9FjXXp6gy4w&feature=youtu.be){:target="_blank"} 

- **M.Sc. in Mathematics (2016-2018)**
    - **Institution**: Universidade de São Paulo, ICMC-USP
    - **Location**: São Paulo, Brazil
    - **Dissertation**: Teoria ergódica em fluxos homogêneos e teoremas de Ratner

- **Bachelor in Mathematics (2012-2015)**
    - **Institution**: Universidade de São Paulo, ICMC-USP
    - **Location**: São Paulo, Brazil

Work experience
======

- **Assistant Professor of Statistics (2024 - Present)**
  - **Institution**: UFSCar, São Carlos/SP, Brazil
  - **Role**: Assistant Professor at the Department of Statistics of the Federal University of São Carlos (UFSCar), Brazil.


- **Postdoctoral Researcher (2024)**
  - **Institution**: ICMC-USP, São Carlos/SP, Brazil
  - **Funding**: FAPESP
  - **Role**: Mapping urban segments and crime types using satellite imagery, spatial metrics, and neural networks to understand socioeconomic and cultural influences on criminal dynamics.


- **Data Scientist Specialist (2023)**
  - **Company**: Samsung Research Brazil, SRBR
  - **Role**: Developed healthcare initiatives for Samsung devices, working with time-series data and sensor information to create AI solutions enhancing user experiences and well-being.

- **Postdoctoral Researcher (2023)**
  - **Institution**: Instituto de Matemática Pura e Aplicada - IMPA
  - **Funding**: FAPERJ
  - **Role**: Conducted machine learning research, led projects, and authored scientific papers.

- **AI Consultant (2023)**
  - **Client**: Rio de Janeiro City Hall (partnership with IMPA)
  - **Role**: Utilized radar and satellite data for short-term rainfall and flood predictions using classical machine learning models. [Project details](https://impa.br/en_US/noticias/impa-desenvolve-algoritmo-para-prever-chuvas-no-rio/){:target="_blank"}.

- **AI Consultant (2022)**
  - **Client**: Cartesi Blockchain (partnership with IMPA)
  - **Role**: Enhanced Cartesi Blockchain's Proof-of-Stake lottery system using computational simulations and statistical theory.

- **AI Consultant (2022)**
  - **Client**: Rede Globo (partnership with IMPA)
  - **Role**: Developed algorithms to extract keywords from over 1.5 million movies and TV series for recommendation systems. [Project details](https://impa.br/noticias/centro-pi-desenvolve-projeto-de-inteligencia-artificial-em-parceria-com-a-globo/){:target="_blank"}.

- **AI Consultant (2021)**
  - **Client**: Grupo Dasa (partnership with IMPA)
  - **Role**: Developed an algorithm to analyze MRI images for detecting pregnancy-related issues with a 93% accuracy rate, aiding early diagnosis in remote areas. [Project details](https://www1.folha.uol.com.br/ciencia/2021/09/inteligencia-artificial-criada-por-matematicos-brasileiros-previne-doencas-na-gravidez.shtml){:target="_blank"}.

- **AI Consultant (2020)**
  - **Client**: Stone Pagamentos (partnership with IMPA)
  - **Role**: Developed a machine learning algorithm to enhance credit decisions, unlocking market potential and solving a critical challenge for the company. [Project details](https://impa.br/noticias/impa-e-stone-matematica-e-industria-para-concessao-de-credito/){:target="_blank"}.

Publications
======
  <ul>{% for post in site.publications reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>

  
<!-- Skills -->
<!-- ====== -->
<!-- * Skill 1 -->
<!-- * Skill 2 -->
<!--   * Sub-skill 2.1 -->
<!--   * Sub-skill 2.2 -->
<!--   * Sub-skill 2.3 -->
<!-- * Skill 3 -->

  
<!-- Talks -->
<!-- ====== -->
<!--   <ul>{% for post in site.talks reversed %} -->
<!--     {% include archive-single-talk-cv.html  %} -->
<!--   {% endfor %}</ul> -->
  
<!-- Teaching -->
<!-- ====== -->
<!--   <ul>{% for post in site.teaching reversed %} -->
<!--     {% include archive-single-cv.html %} -->
<!--   {% endfor %}</ul> -->
  
