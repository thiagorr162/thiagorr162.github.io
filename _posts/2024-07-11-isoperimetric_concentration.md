...
Therefore, we reduce the problem of estimating $(\star)$ to estimating the quantity $P(d(X,B)\geq t)$ where $B$ is a event with probability at least $1/2$ by the definition of  $M(F)$.  Indeed, we can upper bound the above expression as
